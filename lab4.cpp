...
//stiffness1 = rnd() * 0.2 + 0.1;
    ...
    for (int i=0; i<16; i++)
    {
        ...
        if ((i+1)%4 !=0)
        {
            ...
            spring[nsprings].length = 40; //mass[j].pos[0] - mass[i].pos[0];
            spring[nsprings].stiffness = stiffness1;
            nsprings++;
        ...
        {
            spring[nsprings].mass[0] = i;
            spring[nsprings].mass[1] = i+5;
            spring[nsprings].length = 40*sqrt(2); //mass[j].pos[0] - mass[i].pos[0];
            spring[nsprings].stiffness = stiffness1;
            nsprings++;
    ...
    for (int i=0; i<16; i++)
    {
        ...
        {
            spring[nsprings].mass[0] = i;
            spring[nsprings].mass[1] = i+2;
            spring[nsprings].length = 80; //mass[j].pos[0] - mass[i].pos[0];
            spring[nsprings].stiffness = stiffness1;
            nsprings++;
        }
        /*
    ...
    spring[nsprings].length = sqrt(28800); //mass[j].pos[0] - mass[i].pos[0];
    spring[nsprings].stiffness = stiffness1;
    nsprings++;
...
void check_mouse(XEvent *e)
{
	static int savex = 0;
	static int savey = 0;
	static int lbuttonDown=0;//int mx = e->xbutton.x;
	int mx = e->xbutton.x;
    int my = e->xbutton.y;
    //
...
void showMenu()
{
	...
	y += 16;
	x11.drawString(10, y, "+/- to change the stiffness");
	y += 16;
